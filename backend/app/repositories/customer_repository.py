...
class CustomerRepository:
    ...
    def list(self, page: int, page_size: int, shop_key: str, search: str | None = None) -> tuple[list[Customer], int]:
        query = self.db.query(Customer).filter(Customer.is_deleted.is_(False), shop_filter(self.db, Customer, shop_key))
    ...
    def get_by_id(self, customer_pk: int, shop_key: str, include_deleted: bool = False) -> Customer | None:
        ...
        return query.filter(Customer.id == customer_pk, shop_filter(self.db, Customer, shop_key)).first()
    ...
    def exists_business_id(self, customer_business_id: str, shop_key: str | None = None) -> bool:
        ...
        if shop_key is not None:
            query = query.filter(shop_filter(self.db, Customer, shop_key))
    ...
    def create(self, customer: Customer) -> Customer:
        assign_shop_scope(customer, self.db, customer.shop_key)
