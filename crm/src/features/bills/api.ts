...
export async function getBillPdfPreviewUrl(billId: number): Promise<string> {
  const response = await apiClient.get<Blob>(`/bills/${billId}/pdf/download`, {
    responseType: "blob"
  });
  return URL.createObjectURL(response.data);
}

export async function downloadBillPdf(billId: number, fileName?: string): Promise<void> {
  const response = await apiClient.get<Blob>(`/bills/${billId}/pdf/download`, {
    responseType: "blob"
  });
  ...
  anchor.download = fileName ?? `bill-${billId}.pdf`;
