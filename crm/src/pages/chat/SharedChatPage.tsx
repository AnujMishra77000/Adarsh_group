  ...
  useChatSocket({
    ...
    onEvent: (event: ChatSocketEvent) => {
      if (event.event === "chat.message.created") {
        if (currentUser?.shop_key && event.data.sender_shop_key !== currentUser.shop_key) {
          return;
        }
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-2xl border border-pink-300/25 bg-matte-850/90 p-4 shadow-neon-ring sm:p-5">
        <h2 className="text-xl font-semibold text-slate-100">Shop Chat</h2>
        <p className="mt-1 text-sm text-slate-300">
          Private live chat for the selected center. Send text, PDFs, prescriptions, bills, or other files.
