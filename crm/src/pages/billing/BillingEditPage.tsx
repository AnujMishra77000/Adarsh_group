...
import type { Bill, BillItemPayload, BillPayload, BillPaymentPayload } from "@/types/bill";
...
const defaultValues: BillFormValues = {
  customer_id: 0,
  items: [{ item_type: "frame", item_name: "", quantity: 1, unit_price: 0, discount: 0 }],
...
function valuesFromBill(bill: Bill): BillFormValues {
  const items: BillFormValues["items"] =
    bill.items.length > 0
      ...
      : [
          {
            item_type: bill.frame_name ? "frame" : "other",
            item_name: bill.product_name,
            quantity: 1,
            unit_price: Number(bill.whole_price),
  ...
  const payments =
    bill.payments.length > 0
      ...
      : [{ mode: bill.payment_mode, amount: Number(bill.paid_amount), paid_at: "", reference_no: "" }];
...
export function BillingEditPage() {
  ...
  useEffect(() => {
    ...
    reset(valuesFromBill(billQuery.data));
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (bill) => {
      ...
      navigate(CRM_PATHS.billingRecords, { replace: true });
    },
  ...
  const onSubmit = (values: BillFormValues) => {
    ...
    updateMutation.mutate({ id: billId, payload: buildBillPayload(values) });
  };
      ...
      {billQuery.data && (
        <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
          <div className="space-y-2">
            <input value={customerLookup} onChange={(event) => setCustomerLookup(event.target.value)} placeholder="Search customer by ID or contact" className="w-full" />
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-xs font-medium uppercase tracking-wide text-slate-300">Tax Total</label>
              <input type="number" step="0.01" {...register("tax_total")} className="w-full" />
            </div>
            <div>
