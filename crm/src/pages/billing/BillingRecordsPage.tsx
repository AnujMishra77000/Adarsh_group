          ...
          <table className="w-full min-w-[980px] text-left text-sm">
            <thead className="border-b border-pink-400/20 text-xs uppercase tracking-wide text-slate-400">
              <tr>
                ...
                <th className="py-3 pr-3">Grand</th>
            ...
            <tbody>
              {billsQuery.data.items.map((bill) => (
                <tr key={bill.id} className="border-b border-slate-800/80 text-slate-200">
                  ...
                  <td className="py-3 pr-3">{formatCurrency(Number(bill.grand_total))}</td>
                  <td className="py-3 pr-3">{formatCurrency(Number(bill.paid_total))}</td>
                  ...
                  <td className="py-3 pr-3">
                    <div className="flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() => navigate(`${CRM_PATHS.billing}/view/${bill.id}`)}
                      ...
                      {canUpdateBills && (
                        <button
                          type="button"
                          onClick={() => navigate(`${CRM_PATHS.billing}/edit/${bill.id}`)}
                      ...
                      <button
                        type="button"
                        onClick={() => navigate(`${CRM_PATHS.billing}/view/${bill.id}?focus=pdf`)}
