...
import { fetchMe, getAuthConfig, login } from "@/features/auth/api";
...
export function LoginPage({ mode }: LoginPageProps) {
  ...
  const adminRegistrationEnabled = authConfigQuery.data?.admin_registration_enabled === true;
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (tokens) => {
      ...
      try {
        ...
        navigate(from && from !== "/" ? from : CRM_PATHS.dashboard, { replace: true });
      } catch (error) {
        clearAuthTokens(mode);
        clearCurrentUser(mode);
    ...
    <div>
      ...
      <form className="mt-6 space-y-4" onSubmit={handleSubmit((values) => mutation.mutate(values))}>
        <div>
          ...
          <input
            ...
            placeholder={mode === "admin" ? "admin@adarsh-optical.local" : "staff@adarsh-optical.local"}
          />
      ...
      <div className="mt-5 space-y-2 text-sm text-slate-200">
        <p>
          ...
          <Link className="text-pink-100 hover:text-pink-50" to={`${CRM_PATHS.login}/${alternateMode}`}>
            Login as {alternateMode === "admin" ? "Admin" : "Staff"}
          </Link>
        </p>

        {mode === "admin" && adminRegistrationEnabled && (
          <p>
            No admin account yet?{" "}
            <Link className="text-pink-100 hover:text-pink-50" to={CRM_PATHS.adminRegister}>
              Register Admin
            </Link>
          </p>
        )}
        ...
        <p>
          ...
          <Link className="text-pink-100 hover:text-pink-50" to={CRM_PATHS.root}>
            Go to Landing
          </Link>
        </p>
      </div>
