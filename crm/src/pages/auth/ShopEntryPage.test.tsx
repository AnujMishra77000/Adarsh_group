...
describe("ShopEntryPage", () => {
  ...
  it("allows selecting a center directly without typing a mobile number", async () => {
    mockedResolveShop.mockResolvedValue({
    ...
    const { user } = renderWithProviders(
      <Routes>
        <Route path={CRM_PATHS.shopEntry} element={<ShopEntryPage />} />
        <Route path={`${CRM_PATHS.shopResolver}/adarsh-optometric-clinic`} element={<div>Clinic route</div>} />
      </Routes>,
      { route: CRM_PATHS.shopEntry }
    );

    await user.click(screen.getByRole("button", { name: /use adarsh optometric clinic/i }));

    await waitFor(() => {
      expect(mockedResolveShop).toHaveBeenCalled();
    });
    expect(mockedResolveShop.mock.calls[0][0]).toEqual({ identifier: "adarsh-optometric-clinic" });
    expect(sessionStorage.getItem("eye_boutique_active_shop_key")).toBe("adarsh-optometric-clinic");
    expect(await screen.findByText("Clinic route")).toBeInTheDocument();
  });
});
