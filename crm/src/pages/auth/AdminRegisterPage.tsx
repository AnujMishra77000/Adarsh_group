...
import { getAuthConfig, registerAdmin } from "@/features/auth/api";
...
export function AdminRegisterPage() {
  ...
  const registrationEnabled = authConfigQuery.data?.admin_registration_enabled === true;

  const mutation = useMutation({
    mutationFn: registerAdmin,
    onSuccess: () => {
      toast.success("Admin registered successfully");
      navigate(CRM_PATHS.loginAdmin, { replace: true });
    },
    onError: (error) => {
  ...
  if (authConfigQuery.isLoading) {
    return (
      <div>
        ...
        <p className="mt-1 text-sm text-slate-300">Checking registration availability...</p>
      </div>
    );
  }
  ...
  if (!registrationEnabled) {
    return (
      <div>
        <h2 className="text-2xl font-semibold text-slate-100">Admin Registration</h2>
        <p className="mt-1 text-sm text-slate-300">Admin registration is disabled for this environment.</p>
        <div className="mt-5 space-y-2 text-sm text-slate-200">
          <p>
            Already registered?{" "}
            <Link className="text-pink-100 hover:text-pink-50" to={CRM_PATHS.loginAdmin}>
              Login as Admin
            </Link>
          </p>
          <p>
